      ...
      {finalSuggestion ? (
        ...
        <form onSubmit={sendMessage} className="flex mt-4 gap-2">
          <input
            ...
            className="flex-grow border border-gray-300 rounded-l px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-400 shadow-sm"
          />
          <button className="bg-gradient-to-r from-green-500 to-green-700 text-white px-4 py-2 rounded-r hover:scale-105 hover:bg-green-600 transition-all font-semibold shadow">Send</button>
        </form>
      )}
    </div>
  );
